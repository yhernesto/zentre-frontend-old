...
const questionSelectors = Object.assign(
  [],
  generalSettings.clientQuestions.map((e) => {
    return e.question;
  })
);
...
for (let i = 0; i < questionSelectors.length; i++) {
  //NOTE: I do this to get the full object for Question, cause 'questionSelectors' contains only the string word
  const currentCompleateQuestion = generalSettings.clientQuestions.find(
...
const userCountry = app.user.profile?.country.substring(0, 2);
